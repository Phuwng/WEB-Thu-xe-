...
frontend_folder = os.path.abspath(os.path.join(basedir, '..', 'frontend'))
...
def init_db():
    ...
    # Tương thích schema cho database cũ
    ensure_column('vehicles', 'image_url', 'image_url TEXT')
    ensure_column('vehicles', 'is_approved', 'is_approved INTEGER DEFAULT 0')
    ensure_column('vehicles', 'owner_name', 'owner_name TEXT')
    ensure_column('vehicles', 'odo', 'odo INTEGER')
    ensure_column('vehicles', 'brand', 'brand TEXT')
    ensure_column('vehicles', 'description', 'description TEXT')
    ensure_column('vehicles', 'status_reason', 'status_reason TEXT')

    ensure_column('users', 'cccd', 'cccd TEXT')
    ensure_column('users', 'address', 'address TEXT')
...
def confirm_payment(id):
    ...
    try:
        ...
        # Chỉ chặn khi có đơn paid khác đang giữ cùng xe (không chặn chính đơn hiện tại)
        conflict = conn.execute(
            "SELECT id FROM orders WHERE vehicle_id=? AND id!=? AND payment_status='paid' AND status!='completed'",
            (order['vehicle_id'], id)
        ).fetchone()
        if conflict:
...
def return_vehicle(id):
    ...
    try:
        ...
        # Cập nhật order status = completed, số ngày thực tế và số tiền hoàn lại
        actual_return_days = return_days if return_days is not None else order['days']
        conn.execute(
            "UPDATE orders SET status='completed', refund_amount=?, return_days=? WHERE id=?",
            (refund_amount, actual_return_days, id)
        )
...
def admin_get_rented_vehicles():
    ...
    query = '''SELECT v.id, v.name, v.image_url, v.price, v.owner_name,
                      o.id as order_id, o.days, o.total_price, o.rent_date, o.return_days, o.payment_status,
               ...
               JOIN orders o ON v.id = o.vehicle_id
                            AND o.id = (
                                SELECT o2.id
                                FROM orders o2
                                WHERE o2.vehicle_id = v.id AND o2.status != 'completed'
                                ORDER BY o2.rent_date DESC, o2.id DESC
                                LIMIT 1
                            )
               JOIN users u ON o.user_id = u.id 
               WHERE v.status = 'unavailable' AND o.status != 'completed'
               ORDER BY o.rent_date DESC'''
...
@app.route('/owners/<int:user_id>/renting', methods=['GET'])
def get_owner_renting(user_id):
    conn = get_db_connection()
    try:
        user = conn.execute("SELECT fullname, username FROM users WHERE id=?", (user_id,)).fetchone()
        if not user:
            return jsonify({"message": "Người dùng không tồn tại."}), 404
        ...
        # owner_name hiện đang lưu dạng text, nên đối chiếu theo fullname/username
        query = '''SELECT o.id as order_id, o.user_id as renter_id, o.days, o.total_price, o.rent_date,
                          u.fullname as renter_name, u.username as renter_username,
                          v.id as vehicle_id, v.name, v.image_url, v.price as price_per_day, v.status
                   FROM vehicles v
                   JOIN orders o ON o.vehicle_id = v.id
                   JOIN users u ON u.id = o.user_id
                   WHERE o.payment_status = 'paid' AND o.status != 'completed'
                     AND (LOWER(TRIM(v.owner_name)) = LOWER(TRIM(?)) OR LOWER(TRIM(v.owner_name)) = LOWER(TRIM(?)))
                   ORDER BY o.rent_date DESC'''
        rows = conn.execute(query, (fullname, username)).fetchall()
        return jsonify([dict(row) for row in rows])
    finally:
        conn.close()
...
def admin_stats():
    conn = get_db_connection()
    # Doanh thu đã ghi nhận thanh toán (bao gồm đơn đang thuê và đã hoàn tất)
    total_revenue = conn.execute("SELECT SUM(total_price - refund_amount) FROM orders WHERE payment_status='paid'").fetchone()[0] or 0
    completed_revenue = conn.execute("SELECT SUM(total_price - refund_amount) FROM orders WHERE payment_status='paid' AND status='completed'").fetchone()[0] or 0
...
def serve_frontend_files(filename):
    # Only serve known frontend files here; API/static routes are defined above.
